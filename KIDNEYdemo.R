rm(list=ls()) 
DataSet="KIDNEY"  	#Data set name (if any)
n=737			#Number of samples in the data set
...
source("MiMIC.R")
Algo="MiMIC"
mimic=ManifoldBestMinimize(Data=LogData,K=K,rank=6,mod=modalities)


#Perform K-means clustering on joint subspace
UbestSub=as.matrix(read.table("UbestStar.txt",sep=" ",header=FALSE))
cat("\n First few rows of Ubest* subspace:\n")
print(UbestSub[1:5,])
